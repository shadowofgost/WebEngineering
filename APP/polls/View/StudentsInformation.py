...
from .Public import responses_success, responses_fail, get_request_args, data_total_response, content_type_tmp,  patch_error, patch_success, post_search, post_error, put_error, put_success, id_error, data_base_error_specific, delete_schema
...
class StudentsInformation(APIView):
    ...
    def get(self, request, args, session):
        ...
        data_user = models.TCyuser.objects.filter(attr=4).values('id', 'nocard', 'nouser', 'name', 'psw', 'deptid__name', 'sex', 'attr', 'timeupdate', 'userex_related_to_user_information__rem', 'localid', 'userex_related_to_user_information__timeupdate', 'userex_related_to_user_information__idmanager__name'
                                                                 )
    ...
    def post(self, request, args, session):
        ...
        if input_string == None:
            data_user_information = models.TCyuser.objects.filter(attr=4).values('id', 'nocard', 'nouser', 'name', 'psw', 'deptid__name', 'sex', 'attr', 'timeupdate','userex_related_to_user_information__rem', 'localid', 'userex_related_to_user_information__timeupdate', 'userex_related_to_user_information__idmanager__name')
            ...
            if isinstance(test_input, int):
                ...
                ).values('id', 'nocard', 'nouser', 'name', 'psw', 'deptid__name', 'sex', 'attr', 'timeupdate', 'userex_related_to_user_information__rem', 'localid', 'userex_related_to_user_information__timeupdate', 'userex_related_to_user_information__idmanager__name').distinct()
            else:
                data_user_information = models.TCyuser.objects.filter(
                    ...
                    Q(userex_related_to_user_information__rem=input_string), id__attr=4
                ).values('id', 'nocard', 'nouser', 'name', 'psw', 'deptid__name', 'sex', 'attr', 'timeupdate', 'userex_related_to_user_information__rem', 'localid', 'userex_related_to_user_information__timeupdate', 'userex_related_to_user_information__idmanager__name').distinct()
        return data_total_response(data_user_information, pages, limits)
    ...
    def put(self, request, args, session):
        ...
        try:
            ...
            user_id_object = models.TCyuser.objects.get(
                id=variable_name.get('id'))
    ...
    def patch(self, request, args, session):
        ...
        variable_name['idmanager'] = user_id
        try:
            use_tmp = models.TCyuser.objects.get(id=variable_name["id"]).update(
                nocard=variable_name.get('nocard'),
                nouser=variable_name.get('nouser'),
                name=variable_name.get('name'),
                psw=variable_name.get('psw'),
            ...
            tmp = list(models.TCyuserex.objects.filter(id=id).values())
            if tmp == []:
                try:
                    deptid_object = models.TCydept.objects.get(
                        id=variable_name.get('deptid'))
                    ...
                    return HttpResponse(dumps({'error_code': 1, 'message': data_base_error_specific+str(error)}),  content_type=content_type_tmp, charset='utf-8')
            use_tmp_1 = models.TCyuserex.objects.filter(id=variable_name["id"]).update(
                rem=variable_name.get(
                    'userex_related_to_user_information__rem'),
