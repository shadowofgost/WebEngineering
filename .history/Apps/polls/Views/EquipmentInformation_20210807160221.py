...
class EquipmentInformation(APIView):
    ...
    def get(self, request, args, session):
        ...
        data_equipment = models.EquipmentModel.objects.all().values(
    ...
    def post(self, request, args, session):
        ...
        if input_string == None:
            data_equipment = models.EquipmentModel.objects.all().values(
                'id', 'name', 'id_location__name', 'id_location_sn', 'id_ip', 'mac', 'state', 'login', 'link', 'class_field', 'dx', 'dy', 'id_user__name', 'id_plan', 'itimebegin', 'itimelogin', 'whitelist', 'rem', 'timeupdate', 'idmanager__name', 'portlisten', 'type_field', 'timedelay', 'keycancel', 'keyOk', 'keydel', 'keyf1', 'onall', 'rangeequs', 'listplaces').distinct().order_by('id')
        else:
            ...
            if isinstance(test_input, int):
                data_equipment = models.EquipmentModel.objects.filter(
                    Q(id=test_input) |
                    ...
                    Q(keyf1=test_input) |
                    Q(onall=test_input)
                ).values('id', 'name', 'id_location__name', 'id_location_sn', 'id_ip', 'mac', 'state', 'login', 'link', 'class_field', 'dx', 'dy', 'id_user__name', 'id_plan', 'itimebegin', 'itimelogin', 'whitelist', 'rem', 'timeupdate', 'idmanager__name', 'portlisten', 'type_field', 'timedelay', 'keycancel', 'keyOk', 'keydel', 'keyf1', 'onall', 'rangeequs', 'listplaces').distinct().order_by('id')
            else:
                data_equipment = models.EquipmentModel.objects.filter(
                    Q(name__icontains=test_input) |
                    Q(whitelist__icontains=test_input) |
    ...
    def put(self, request, args, session):
        ...
        try:
            ...
            user_object = models.UserModel.objects.get(
                id=variable_name.get('id_user'))
            plan_object = models.CoursePlanModel.objects.get(
                id=variable_name.get('id_plan'))
            idmanager_object = models.UserModel.objects.get(
                id=variable_name.get('idmanager'))
            ueses_tmp = models.EquipmentModel.objects.create(
    ...
    def patch(self, request, args, session):
        ...
        data_equipment_initial = list(
            models.EquipmentModel.objects.filter(id=id_equipment).values('id', 'name', 'id_location', 'id_location_sn',    'id_ip', 'mac', 'state', 'login', 'link', 'class_field', 'dx', 'dy', 'id_user', 'id_plan', 'itimebegin', 'whitelist', 'rem', 'timeupdate', 'idmanager', 'portlisten', 'type_field', 'timedelay', 'keycancel', 'keyOk', 'keydel', 'keyf1', 'onall', 'rangeequs', 'listplaces'))
        ...
        variable_name['idmanager'] = user_id
        try:
            models.EquipmentModel.objects.filter(id=id_equipment).update(
                name=variable_name.get('name'),
