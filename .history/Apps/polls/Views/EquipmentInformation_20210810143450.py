...
class EquipmentInformation(APIView):
    ...
    def put(self, request, args, session):
        ...
        if isinstance(variable_name["id_location__name"], int) and isinstance(
            variable_name["id_user__name"], int
        ):
            variable_name["id_location"] = variable_name["id_location__name"]
            variable_name["id_user"] = variable_name["id_user__name"]
        else:
        ...
        try:
            ...
            plan_object = CoursePlanModel.objects.get(id=variable_name.get("id_plan"))
            idmanager_object = UserModel.objects.get(id=variable_name.get("idmanager"))
            ueses_tmp = EquipmentModel.objects.create(
            ...
            return HttpResponse(
                dumps({"message": put_success}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
        except Exception as error:
            return HttpResponse(
                dumps({"message": data_base_error_specific + str(error)}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
    ...
    EquipmentInformation_patch_responses_success = Response(
        ...
        schema=responses_success,
        examples={"error_code": 0, "message": "修改成功"},
    )
    ...
    def patch(self, request, args, session):
        ...
        data_equipment_initial = list(
            EquipmentModel.objects.filter(id=id_equipment).values(
                "id",
                "name",
                "id_location",
                "id_location_sn",
        ...
        args["id_user"] = args.get("id_user__name", data_equipment["id_user"])
        args["idmanager"] = args.get("idmanager__name", data_equipment["idmanager"])
        if isinstance(args["id_location__name"], int) and isinstance(
            args["id_user__name"], int
        ):
        ...
        variable_name["idmanager"] = user_id
        try:
            EquipmentModel.objects.filter(id=id_equipment).update(
                name=variable_name.get("name"),
            ...
            return HttpResponse(
                dumps({"error_code": 0, "message": "修改设备信息成功"}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
        except Exception as error:
