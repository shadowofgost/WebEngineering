...
class AttendanceInformation(APIView):
    ...
    data_schema_personal_data = {
        ...
        "param2__id_curricula__name": Schema(
            title="签到课程名称",
            description="签到课程名称",
            type=TYPE_STRING,
            format="string",
    ...
    AttendanceInformation_get_responses_success = Response(
        ...
        examples={"error_code": 0, "message": "获取成功"},
    )
    AttendanceInformation_get_responses_fail = Response(
        description="获取签到记录失败的响应",
        schema=responses_fail,
        examples={"error_code": 1, "message": "获取失败"},
    )

    @swagger_auto_schema(
        request_body=None,
        manual_parameters=[
            AttendanceInformation_get_parammeter,
            page_get_parammeter,
    ...
    def get(self, request, args, session):
        ...
        if data_equipment == []:
            return HttpResponse(
                dumps(
                    {"error_code": 1, "message": "所要查询的记录不存在，请确认是否课程是否开课，其他意外情况请联系教务部"}
                ),
    ...
    @swagger_auto_schema(
        ...
        security=None,
        responses={
            201: AttendanceInformation_post_responses_success,
            400: AttendanceInformation_post_responses_fail,
        },
        tags=None,
    )
    @get_request_args
    def post(self, request, args, session):
        ...
        if input_string == None:
            ...
            if isinstance(test_input, int):
                data_equipment = list(
                    RunningAccountModel.objects.filter(
                        Q(id=test_input)
                        | Q(id_user=test_input)
                        | Q(param2=test_input)
                        | Q(idmanager=test_input)
                        | Q(time=test_input)
                        | Q(timeupdate=str(test_input)),
    ...
    AttendanceInformation_put_schema["timeupdate"] = data_schema_personal_data[
        "timeupdate"
    ]
    ...
    def put(self, request, args, session):
        ...
        # 批量命名变量
        try:
            user_object = UserModel.objects.get(id=variable_name.get("id_user"))
            param2_object = CoursePlanModel.objects.get(id=variable_name.get("param2"))
            idmanager_object = UserModel.objects.get(id=variable_name.get("idmanager"))
            ueses_tmp = RunningAccountModel.objects.create(
    ...
    @swagger_auto_schema(
        ...
        security=None,
        responses={
            201: AttendanceInformation_patch_responses_success,
            400: AttendanceInformation_patch_responses_fail,
        },
        tags=None,
    )
    @get_request_args
    def patch(self, request, args, session):
        ...
        data_equipment_initial = list(
            RunningAccountModel.objects.filter(id=id_equipment).values(
                "id",
                "id_user",
                "param2",
                "idmanager",
        ...
        variable_name["idmanager"] = user_id
        try:
            RunningAccountModel.objects.filter(id=id_equipment).update(
                id=variable_name.get("id"),
            ...
            return HttpResponse(
                dumps({"error_code": 0, "message": "修改签到记录成功"}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
        except Exception as error:
