...
class IDEvaluation(APIView):
    ...
    def post(self, request, args, session):
        ...
        if kind_evaluation == 1:
            data = list(UserModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 2:
            data = list(EquipmentModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 3:
            data = list(CurriculaModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 4:
            data = list(RunningAccountModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 5:
            data = list(DepartmentModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 6:
            data = list(LocationModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 7:
            data = list(CoursePlanModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 8:
            data = list(MmxModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 9:
            data = list(TyperaModel.objects.filter(
                id=id_evaluation).values())
        if data == []:
