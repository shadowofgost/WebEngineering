...
class TeachersInformation(StudentsInformation):
    ...
    def get(self, request, args, session):
        ...
        data_user = UserModel.objects.filter().exclude(attr=4).values('id', 'nocard', 'nouser', 'name', 'psw', 'deptid__name', 'sex', 'attr', 'timeupdate', 'userex_related_to_user_information__rem',
    ...
    def post(self, request, args, session):
        ...
        if input_string == None:
            data_user_information = UserModel.objects.filter().exclude(attr=4).values('id', 'nocard', 'nouser', 'name', 'psw', 'deptid__name', 'sex', 'attr', 'timeupdate',
