...
class StudentsInformation(APIView):
    ...
    def get(self, request, args, session):
        ...
        data_user = models.UserModel.objects.filter(attr=4).values('id', 'nocard', 'nouser', 'name', 'psw', 'deptid__name', 'sex', 'attr', 'timeupdate', 'userex_related_to_user_information__rem',
    ...
    def post(self, request, args, session):
        ...
        if input_string == None:
            data_user_information = models.UserModel.objects.filter(attr=4).values('id', 'nocard', 'nouser', 'name', 'psw', 'deptid__name', 'sex', 'attr', 'timeupdate', 'userex_related_to_user_information__rem',
                                                                                 'localid', 'userex_related_to_user_information__timeupdate', 'userex_related_to_user_information__idmanager__name').distinct().order_by('id')
            ...
            if isinstance(test_input, int):
                data_user_information = models.UserModel.objects.filter(
                    Q(id=test_input) |
    ...
    def put(self, request, args, session):
        ...
        try:
            deptid_object = models.DepartmentModel.objects.get(
                id=variable_name.get('deptid'))
            use_tmp = models.UserModel.objects.create(
            ...
            user_id_object = models.UserModel.objects.get(
                id=variable_name.get('id'))
            use_tmp_1 = models.UserExtensionModel.objects.create(
                id=user_id_object,
    ...
    def patch(self, request, args, session):
        ...
        data_user_information_initial = list(models.UserModel.objects.filter(id=id_user_information).values(
        ...
        variable_name['idmanager'] = user_id
        try:
            use_tmp = models.UserModel.objects.get(id=variable_name["id"]).update(
                nocard=variable_name.get('nocard'),
                nouser=variable_name.get('nouser'),
                name=variable_name.get('name'),
                psw=variable_name.get('psw'),
            ...
            tmp = list(models.UserExtensionModel.objects.filter(id=id).values())
            if tmp == []:
                try:
                    deptid_object = models.DepartmentModel.objects.get(
                        id=variable_name.get('deptid'))
                    ...
                    return HttpResponse(dumps({'error_code': 1, 'message': data_base_error_specific+str(error)}),  content_type=content_type_tmp, charset='utf-8')
            use_tmp_1 = models.UserExtensionModel.objects.filter(id=variable_name["id"]).update(
                rem=variable_name.get(
                    'userex_related_to_user_information__rem'),
    ...
    def delete(self, request, args, session):
        ...
        try:
            for i in range(numbers_id):
                models.UserExtensionModel.objects.filter(
