...
class Picture(APIView):
    ...
    post_responses_success = Response(
        ...
        examples={"error_code": 0, "message": "上传图片成功"},
    )
    post_responses_fail = Response(
        description="上传图片失败的响应",
        schema=responses_fail,
        examples={"error_code": 1, "message": "上传图片失败"},
    )
    ...
    @swagger_auto_schema(
        ...
        security=None,
        responses={200: post_responses_success, 401: post_responses_fail},
        tags=None,
    )
    @get_request_args
    def post(self, request):
        ...
        try:
            ...
            return HttpResponse(
                dumps({"error_code": 0, "message": "添加数据成功"}),
