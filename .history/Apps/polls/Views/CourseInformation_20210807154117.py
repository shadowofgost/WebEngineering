...
class CourseInformation(APIView):
    ...
    def get(self, request, args, session):
        ...
        data_equipment = []
        if user_group_id == 3:
            data_equipment = models.CurriculaModel.objects.filter(id_speaker=user_id).values(
            ...
            data_equipment = models.CurriculaModel.objects.all().values(
                'id', 'name', 'timebegin', 'timeend', 'id_location__name',
                'id_speaker__name', 'attr', 'charge', 'pwaccess',
                'pwcontinuous', 'pwdirection', 'dooropen', 'timebegincheckbegin',
                'timebegincheckend', 'timeendcheckbegin', 'timeendcheckend',
            ...
            data_equipment = models.CurriculaModel.objects.filter(Q(rangeusers__icontains=user_nouser)).values('id', 'name', 'timebegin', 'timeend', 'id_location__name', 'id_speaker__name', 'attr', 'charge', 'pwaccess', 'pwcontinuous', 'pwdirection', 'dooropen',
    ...
    def post(self, request, args, session):
        ...
        if input_string == None:
            if user_group_id == 3:
                ...
                data_equipment = models.CurriculaModel.objects.filter(Q(rangeusers__icontains=user_nouser)).values(
                    'id', 'name', 'timebegin', 'timeend', 'id_location__name',
                    'id_speaker__name', 'attr', 'charge', 'pwaccess',
                    'pwcontinuous', 'pwdirection', 'dooropen', 'timebegincheckbegin',
                    'timebegincheckend', 'timeendcheckbegin', 'timeendcheckend',
        ...
        # if data_equipment == []:
            ...
            if user_group_id == 3:
                if isinstance(test_input, int):
                    data_equipment = models.CurriculaModel.objects.filter(
                        Q(id=test_input)
                        ...
                        | Q(idmanager=test_input), id_speaker=user_id).values(
                            'id',
                            'name',
                            'timebegin',
                            'timeend',
                    ...
                    data_equipment = models.CurriculaModel.objects.filter(
                        ...
                        | Q(rangeusers__icontains=test_input
                            ...
                            | Q(idmanager__name__icontains=test_input),  id_speaker=user_id)).values(
                        'id',
                        'name',
                        'timebegin',
                        'timeend',
                ...
                if isinstance(test_input, int):
                    ...
                    data_equipment = models.CurriculaModel.objects.filter(
                        ...
                        | Q(mapuser2equ__icontains=test_input)
                        | Q(aboutspeaker__icontains=test_input)
                        | Q(idmanager__name__icontains=test_input)).values(
                            'id',
                            'name',
                            'timebegin',
                            'timeend',
                ...
                if isinstance(test_input, int):
                    data_equipment = models.CurriculaModel.objects.filter(Q(id=test_input) | Q(timebegin=test_input) | Q(timeend=test_input) | Q(id_location=test_input) | Q(id_speaker=test_input) | Q(attr=test_input) | Q(charge=test_input) | Q(pwaccess=test_input) | Q(pwcontinuous=test_input) | Q(pwdirection=test_input) | Q(dooropen=test_input) | Q(timebegincheckbegin=test_input) | Q(timebegincheckend=test_input) | Q(timeendcheckbegin=test_input) | Q(timeendcheckend=test_input) | Q(
                    ...
                    data_equipment = models.CurriculaModel.objects.filter(Q(name__icontains=test_input) | Q(id_location__name__icontains=test_input) | Q(id_speaker__name__icontains=test_input) | Q(idmanager__name__icontains=test_input) | Q(rem__icontains=test_input) | Q(rangeequs__icontains=test_input) | Q(rangeusers__icontains=test_input) | Q(listdepts__icontains=test_input) | Q(listplaces__icontains=test_input) | Q(mapuser2equ__icontains=test_input) | Q(aboutspeaker__icontains=test_input) | Q(
                        idmanager__name__icontains=test_input), Q(rangeusers__icontains=user_nouser)).values('id', 'name', 'timebegin', 'timeend', 'id_location__name', 'id_speaker__name', 'attr', 'charge', 'pwaccess', 'pwcontinuous', 'pwdirection', 'dooropen', 'timebegincheckbegin', 'timebegincheckend', 'timeendcheckbegin', 'timeendcheckend', 'rangeusers', 'listdepts', 'rangeequs', 'timeupdate', 'listplaces', 'idmanager__name', 'mapuser2equ', 'aboutspeaker', 'rem').distinct().order_by('id')
            # elif user_group_id == 4:
                # if isinstance(test_input, int):
    ...
    def put(self, request, args, session):
        ...
        if isinstance(variable_name['id_location__name'], int) and isinstance(variable_name['id_speaker__name'], int):
            variable_name['id_location'] = variable_name['id_location__name']
            variable_name['id_speaker'] = variable_name['id_speaker__name']
        else:
        ...
        # 批量命名变量
        try:
            location_object = models.LocationModel.objects.get(
                id=variable_name.get('id_location'))
            speaker_object = models.UserModel.objects.get(
                id=variable_name.get('id_speaker'))
            idmanager_object = models.UserModel.objects.get(
                id=variable_name.get('idmanager'))
            ueses_tmp = models.CurriculaModel.objects.create(
    ...
    def patch(self, request, args, session):
        ...
        data_equipment_initial = list(
            models.CurriculaModel.objects.filter(id=id_equipment).values('id', 'name', 'timebegin', 'timeend', 'id_location', 'id_speaker', 'attr', 'charge', 'pwaccess', 'pwcontinuous', 'pwdirection', 'dooropen', 'timebegincheckbegin', 'timebegincheckend', 'timeendcheckbegin', 'timeendcheckend', 'rangeusers', 'listdepts', 'rangeequs', 'timeupdate', 'listplaces', 'idmanager', 'mapuser2equ', 'aboutspeaker', 'rem'))
        ...
        if isinstance(args['id_location__name'], int) and isinstance(args['id_speaker__name'], int) and isinstance(args['idmanager__name'], int):
        ...
        variable_name['idmanager'] = user_id
        try:
            models.CurriculaModel.objects.filter(id=id_equipment).update(
                id=variable_name.get('id'),
                name=variable_name.get('name'),
    ...
    def delete(self, request, args, session):
        ...
        try:
            for i in range(numbers_id):
                models.CurriculaModel.objects.filter(
