...
class PersonInformation(APIView):
    ...
    def get(self, request, args, session):
        ...
        data_user = list(models.UserModel.objects.filter(id=user_id).values('id', 'nocard', 'nouser', 'name', 'psw', 'deptid__name', 'sex', 'attr', 'timeupdate',
    ...
    def patch(self, request, args, session):
        ...
        if data_patch_name or data_patch_sex or data_patch_psw:
            data_user = list(
                models.UserModel.objects.filter(id=user_id).values('id', 'nocard', 'nouser', 'name', 'psw', 'sex', 'idmanager', 'timeupdate'))
            if data_user == []:
                ...
                models.UserModel.objects.filter(id=user_id).update(
        ...
        if data_patch_rem:
            data_user_extend = list(
                models.UserExtensionModel.objects.filter(id=user_id).values('id', 'rem', 'idmanager', 'timeupdate'))
            if data_user_extend == []:
                id_object = models.UserModel.objects.get(id=user_id)
                models.UserExtensionModel.objects.create(
                    id=id_object,
                    rem='None',
                ...
                models.UserExtensionModel.objects.filter(id=user_id).update(
