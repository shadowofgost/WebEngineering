...
# @FilePath         : /WebEngineering/Apps/polls/Views/Login.py
# @LastAuthor       : Albert Wang
# @LastTime         : 2022-03-10 20:13:25
# @Software         : Vscode
"""
...
class Login(APIView):
    ...
    login_post_responses_success = Response(
        ...
        examples={"uerid": 1910404051, "psw": "123456", "status": "0"},
    )
    ...
    def post(self, request, args, session):
        if request.session.get("is_login", False):
            ...
            return HttpResponse(
                dumps({"code": 1}), content_type=content_type_tmp, charset="utf-8"
            )
        ...
        if len(str(user_id)) == 0 or len(password) == 0:
            return HttpResponse(
                dumps({"error_code": 1, "message": "请输入用户名和密码"}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
        # select data from database based on the id of the user.
        data = tuple(UserModel.objects.filter(nouser=str(user_id)).values())
        if data == ():
            return HttpResponse(
                dumps({"error_code": 1, "message": "用户名不存在，重新输入用户名或者进行注册"}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
        else:
            ...
            if data_password != password:
                return HttpResponse(
                    dumps({"error_code": 1, "message": "登录失败,密码不正确"}),
                    content_type=content_type_tmp,
                    charset="utf-8",
                )
