...
class CourseInformation(APIView):
    ...
    def get(self, request, args, session):
        ...
        data_equipment = []
        if user_group_id == 3:
            data_equipment = CurriculaModel.objects.filter(id_speaker=user_id).values(
            ...
            data_equipment = CurriculaModel.objects.all().values(
                'id', 'name', 'timebegin', 'timeend', 'id_location__name',
                'id_speaker__name', 'attr', 'charge', 'pwaccess',
                'pwcontinuous', 'pwdirection', 'dooropen', 'timebegincheckbegin',
                'timebegincheckend', 'timeendcheckbegin', 'timeendcheckend',
            ...
            data_equipment = CurriculaModel.objects.filter(Q(rangeusers__icontains=user_nouser)).values('id', 'name', 'timebegin', 'timeend', 'id_location__name', 'id_speaker__name', 'attr', 'charge', 'pwaccess', 'pwcontinuous', 'pwdirection', 'dooropen',
    ...
    def post(self, request, args, session):
        ...
        if input_string == None:
            if user_group_id == 3:
                ...
                data_equipment = CurriculaModel.objects.filter(Q(rangeusers__icontains=user_nouser)).values(
                    'id', 'name', 'timebegin', 'timeend', 'id_location__name',
                    'id_speaker__name', 'attr', 'charge', 'pwaccess',
                    'pwcontinuous', 'pwdirection', 'dooropen', 'timebegincheckbegin',
                    'timebegincheckend', 'timeendcheckbegin', 'timeendcheckend',
        ...
        # if data_equipment == []:
            ...
            if user_group_id == 3:
                if isinstance(test_input, int):
                    data_equipment = CurriculaModel.objects.filter(
                        Q(id=test_input)
    ...
    def patch(self, request, args, session):
        ...
        data_equipment_initial = list(
            CurriculaModel.objects.filter(id=id_equipment).values('id', 'name', 'timebegin', 'timeend', 'id_location', 'id_speaker', 'attr', 'charge', 'pwaccess', 'pwcontinuous', 'pwdirection', 'dooropen', 'timebegincheckbegin', 'timebegincheckend', 'timeendcheckbegin', 'timeendcheckend', 'rangeusers', 'listdepts', 'rangeequs', 'timeupdate', 'listplaces', 'idmanager', 'mapuser2equ', 'aboutspeaker', 'rem'))
