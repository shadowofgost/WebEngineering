...
class EquipmentInformation(APIView):
    ...
    def get(self, request, args, session):
        ...
        data_equipment = EquipmentModel.objects.all().values(
    ...
    def post(self, request, args, session):
        ...
        if input_string == None:
            ...
            if isinstance(test_input, int):
                data_equipment = EquipmentModel.objects.filter(
                    Q(id=test_input) |
    ...
    def put(self, request, args, session):
        ...
        try:
            ...
            user_object = UserModel.objects.get(
                id=variable_name.get('id_user'))
            plan_object = CoursePlanModel.objects.get(
    ...
    def patch(self, request, args, session):
        ...
        data_equipment_initial = list(
            EquipmentModel.objects.filter(id=id_equipment).values('id', 'name', 'id_location', 'id_location_sn',    'id_ip', 'mac', 'state', 'login', 'link', 'class_field', 'dx', 'dy', 'id_user', 'id_plan', 'itimebegin', 'whitelist', 'rem', 'timeupdate', 'idmanager', 'portlisten', 'type_field', 'timedelay', 'keycancel', 'keyOk', 'keydel', 'keyf1', 'onall', 'rangeequs', 'listplaces'))
