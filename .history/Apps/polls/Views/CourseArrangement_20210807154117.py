...
class CourseArrangement(APIView):
    ...
    def get(self, request, args, session):
        ...
        data_equipment = models.CoursePlanModel.objects.all().values('id',
    ...
    def post(self, request, args, session):
        ...
        if input_string == None:
            data_equipment = models.CoursePlanModel.objects.all().values(
                'id',
                'id_curricula__name',
                'timebegin',
                'timeend',
            ...
            if isinstance(test_input, int):
                data_equipment = models.CoursePlanModel.objects.filter(
                    Q(id=test_input)
    ...
    def put(self, request, args, session):
        ...
        # 批量命名变量
        try:
            curricula_object = models.CurriculaModel.objects.get(
            ...
            ueses_tmp = models.CoursePlanModel.objects.create(
    ...
    def patch(self, request, args, session):
        ...
        data_equipment_initial = list(
            models.CoursePlanModel.objects.filter(id=id_equipment).values(
                'id',
                'id_curricula',
                'timebegin',
                'timeend',
        ...
        variable_name['idmanager'] = user_id
        try:
            models.CoursePlanModel.objects.filter(id=id_equipment).update(
                id=variable_name.get('id'),
    ...
    def delete(self, request, args, session):
        ...
        try:
            for i in range(numbers_id):
                models.CoursePlanModel.objects.filter(
