...
class QuitWarning(APIView):
    data_personal = Schema(
        ...
        properties={
            ...
            "id_user__name": Schema(
                title="学生姓名", description="学生的姓名", type=TYPE_STRING, format="string"
            ),
    ...
    data_schema = {
        ...
        "name": Schema(
            title="课程名", description="课程名", type=TYPE_STRING, format="string"
        ),
    ...
    Warning_get_responses_success = Response(
        ...
        examples={"error_code": 0, "message": "成功获取信息"},
    )
    get_responses_fail = Response(
        description="检测图片失败的响应",
        schema=responses_fail,
        examples={"error_code": 1, "message": "获取信息失败失败"},
    )
    ...
    @swagger_auto_schema(
        ...
        security=None,
        responses={200: Warning_get_responses_success, 401: get_responses_fail},
        tags=None,
    )
    @get_request_args
    def get(self, request, args, session):
        ...
        if data_course == []:
            return HttpResponse(
                dumps({"error_code": 1, "message": "数据库出错，请联系管理员"}),
        ...
        if return_data == []:
            return HttpResponse(
                dumps({"error_code": 1, "message": "没有考勤不合格学生"}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
        else:
