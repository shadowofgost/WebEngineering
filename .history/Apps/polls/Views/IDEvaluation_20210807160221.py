...
class IDEvaluation(APIView):
    ...
    def post(self, request, args, session):
        ...
        if kind_evaluation == 1:
            data = list(models.UserModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 2:
            data = list(models.EquipmentModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 3:
            data = list(models.CurriculaModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 4:
            data = list(models.RunningAccountModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 5:
            data = list(models.DepartmentModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 6:
            data = list(models.LocationModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 7:
            data = list(models.CoursePlanModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 8:
            data = list(models.MmxModel.objects.filter(
                id=id_evaluation).values())
        elif kind_evaluation == 9:
            data = list(models.TyperaModel.objects.filter(
                id=id_evaluation).values())
        if data == []:
