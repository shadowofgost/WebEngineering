...
def data_total_response(data, pages, limits):
    ...
    if data_response == []:
        return HttpResponse(
            dumps({"error_code": 1, "message": "内容不存在"}),
            content_type=content_type_tmp,
            charset="utf-8",
        )
    else:
...
def data_attendance_format(data_equipment, data_plan, id_list):
    ...
    for i in data_equipment:
        ...
        if id_user_initial == data_id_user:
            ...
            for j in data_plan:
                if i["param2"] == j["id"]:
                    ...
                    person_data_one["param2__id_curricula__name"] = j[
                        "id_curricula__name"
                    ]
            ...
            person_total_data["id_user__email"] = i["id_user__email"]
            person_total_data["id_user__nouser"] = i["id_user__nouser"]
        ...
        person_total_data["id_user__email"] = data_equipment[-1]["id_user__email"]
        person_total_data["id_user__nouser"] = data_equipment[-1]["id_user__nouser"]
        person_total_data["attendtimes"] = attend_times
        person_total_data["attendtotal"] = attend_total
        person_total_data["attendrates"] = attend_times / attend_total
        person_total_data["person_data"] = person_data
...
def data_attendance_class_format(data_equipment, data_plan, id_list):
    ...
    for i in data_equipment:
        ...
        if id_user_initial == data_id_user:
            ...
            for j in data_plan:
                if i["param2"] == j["id"]:
                    ...
                    person_data_one["id_user__nouser"] = i["id_user__nouser"]
                    person_data_one["timeupdate"] = i["timeupdate"]
                    person_data_one["idmanager__name"] = i["idmanager__name"]
                    person_data_one["time"] = i["time"]
                    if j["timebegincheckbegin"] == None:
        ...
        person_total_data["id"] = id_user_initial
        person_total_data["id_user__name"] = data_equipment[-1]["id_user__name"]
        person_total_data["id_user__nouser"] = data_equipment[-1]["id_user__name"]
        person_total_data["attendtimes"] = attend_times
        person_total_data["attendtotal"] = attend_total
        person_total_data["attendrates"] = attend_times / attend_total
        person_total_data["person_data"] = person_data
...
def data_attendance(course_plan_id, id_list, format_type, user_id):
    ...
    data_plan = [1, 2, 3, 4, 5]
    if format_type == 3:
        data_equipment = tuple(
            RunningAccountModel.objects.filter(
                id_user=user_id, param2__id_curricula=course_plan_id
            )
            .values(
                "id",
                "id_user__name",
        ...
        data_plan = tuple(
            CoursePlanModel.objects.filter(id_curricula=course_plan_id)
            .values(
                "id",
                "timebegin",
                "timeend",
        ...
        data_equipment = tuple(
            RunningAccountModel.objects.filter(param2__id_curricula=course_plan_id)
            .values(
                "id",
                "id_user__name",
                "id_user__email",
...
def select_quit_warning_student(
    course_plan_id,
    id_list,
    format_type,
    user_id,
    limit_not_attendrates,
    limit_not_attendtimes,
):
    """
    这个函数是用来读取每一门课不符合要求的学生名单
    """
    data = data_attendance(course_plan_id, id_list, format_type, user_id)
    select_student = []
    for j in data:
        personal_data = {}
        ...
        if (
            limit_not_attendtimes < attendtotal - attendtimes
            or limit_not_attendrates < 1 - int(limit_not_attendrates)
        ):
