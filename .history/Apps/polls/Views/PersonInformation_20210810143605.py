...
class PersonInformation(APIView):
    ...
    PersonInformation_get_responses_success = Response(
        ...
        schema=Schema(
            title="成功数据", description="成功数据", type=TYPE_OBJECT, properties=data_schema
        ),
        examples=None,
    )
    PersonInformation_get_responses_fail = Response(
        ...
        examples={"error_code": 1, "message": "运行报错"},
    )
    ...
    def get(self, request, args, session):
        ...
        data_user = list(
            UserModel.objects.filter(id=user_id).values(
                "id",
                "nocard",
                "nouser",
                "name",
        ...
        if data_user == []:
            return HttpResponse(
                dumps({"error_code": 1, "message": "数据库出现错误，请联系管理员"}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
        return HttpResponse(
            dumps({"error_code": 0, "data": data_user}),
            content_type=content_type_tmp,
            charset="utf-8",
        )
    ...
    def patch(self, request, args, session):
        ...
        data_patch_rem = args.get("userex_related_to_user_information__rem", None)
        if not (data_patch_name or data_patch_sex or data_patch_rem or data_patch_psw):
            return HttpResponse(
                dumps({"error_code": 0, "message": patch_success}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
        user_id = request.COOKIES.get("user_id")
        ...
        if data_patch_name or data_patch_sex or data_patch_psw:
            ...
            if data_user == []:
                return HttpResponse(
                    dumps({"error_code": 1, "message": data_base_error}),
                    content_type=content_type_tmp,
                    charset="utf-8",
                )
            else:
        ...
        if data_patch_rem:
            data_user_extend = list(
                UserExtensionModel.objects.filter(id=user_id).values(
                    "id", "rem", "idmanager", "timeupdate"
                )
            )
            if data_user_extend == []:
                ...
                data_user_extend["rem"] = args.get("rem", data_user_extend["rem"])
