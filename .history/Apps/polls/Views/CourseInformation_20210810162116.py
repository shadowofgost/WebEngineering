...
class CourseInformation(APIView):
    ...
    @swagger_auto_schema(
        ...
        security=None,
        responses={
            200: CourseInformation_get_responses_success,
            401: CourseInformation_get_responses_fail,
        },
        tags=None,
    )
    @get_request_args
    def get(self, request, args, session):
        ...
        if not request.session.get(is_login, None):
            return HttpResponse(
                dumps({"code": 0}), content_type=content_type_tmp, charset="utf-8"
            )
        ...
        if user_group_id == 3:
            data_equipment = (
                CurriculaModel.objects.filter(id_speaker=user_id)
                .values(
                    "id",
                    "name",
                    "timebegin",
        ...
        elif user_group_id == 1 or user_group_id == 2:
            data_equipment = (
                CurriculaModel.objects.all()
                .values(
                    "id",
                    "name",
                    "timebegin",
            ...
            data_equipment = (
                CurriculaModel.objects.filter(Q(rangeusers__icontains=user_nouser))
                .values(
                    "id",
                    "name",
                    "timebegin",
    ...
    CourseInformation_post_responses_success = Response(
        description="查询课程信息成功的响应",
        schema=get_responses_success,
        examples={"error_code": 0, "message": "查询成功"},
    )
    ...
    def post(self, request, args, session):
        ...
        # data_equipment = CurriculaModel.objects.all().values('id', 'name', 'timebegin', 'timeend', 'id_location__name','id_speaker__name', 'attr', 'charge', 'pwaccess','pwcontinuous', 'pwdirection', 'dooropen','timebegincheckbegin','timebegincheckend', 'timeendcheckbegin', 'timeendcheckend','rangeusers', 'listdepts', 'rangeequs', 'timeupdate', 'listplaces','idmanager__name', 'mapuser2equ', 'aboutspeaker', 'rem').distinct().order_by('id')
                ...
                if isinstance(test_input, int):
                    ...
                    data_equipment = (
                        CurriculaModel.objects.filter(
                            ...
                            | Q(rem__icontains=test_input)
                            | Q(rangeequs__icontains=test_input)
                            | Q(rangeusers__icontains=test_input)
                            | Q(listdepts__icontains=test_input)
                            | Q(listplaces__icontains=test_input)
                            | Q(mapuser2equ__icontains=test_input)
                            | Q(aboutspeaker__icontains=test_input)
                ...
                if isinstance(test_input, int):
                    data_equipment = (
                        CurriculaModel.objects.filter(
                            ...
                            | Q(timeupdate=test_input)
                            | Q(idmanager=test_input),
                            Q(rangeusers__icontains=user_nouser),
                        )
                        .values(
                            "id",
                            "name",
            ...
            # elif user_group_id == 4:
            # if isinstance(test_input, int):
            # data_equipment = CurriculaModel.objects.filter(Q(id=test_input)| Q(timebegin=test_input)| Q(timeend=test_input)| Q(id_location=test_input)| Q(id_speaker=test_input)| Q(attr=test_input)| Q(charge=test_input)| Q(pwaccess=test_input)| Q(pwcontinuous=test_input)| Q(pwdirection=test_input)| Q(dooropen=test_input)| Q(timebegincheckbegin=test_input)| Q(timebegincheckend=test_input)| Q(timeendcheckbegin=test_input)| Q(timeendcheckend=test_input)| Q(timeupdate=test_input)| Q(idmanager=test_input),id_curricula__runningaccount_related_to_plan__id_user = user_id).values('id','name','timebegin','timeend','id_location__name','id_speaker__name','attr','charge','pwaccess','pwcontinuous','pwdirection','dooropen','timebegincheckbegin','timebegincheckend','timeendcheckbegin','timeendcheckend','rangeusers','listdepts','rangeequs','timeupdate','listplaces','idmanager__name','mapuser2equ','aboutspeaker','rem').distinct().order_by('id')
            # else:
            # data_equipment = CurriculaModel.objects.filter(Q(name__icontains=test_input)| Q(id_location__name__icontains=test_input)| Q(id_speaker__name__icontains=test_input)| Q(idmanager__name__icontains=test_input)| Q(rem__icontains=test_input)| Q(rangeequs__icontains=test_input)| Q(rangeusers__icontains=test_input)| Q(listdepts__icontains=test_input)| Q(listplaces__icontains=test_input)| Q(mapuser2equ__icontains=test_input)| Q(aboutspeaker__icontains=test_input)| Q(idmanager__name__icontains=test_input),id_curricula__runningaccount_related_to_plan__id_user = user_id).values('id','name','timebegin','timeend','id_location__name','id_speaker__name','attr','charge','pwaccess','pwcontinuous','pwdirection','dooropen','timebegincheckbegin','timebegincheckend','timeendcheckbegin','timeendcheckend','rangeusers','listdepts','rangeequs','timeupdate','listplaces','idmanager__name','mapuser2equ','aboutspeaker','rem').distinct().order_by('id')
            if data_equipment == []:
                if isinstance(test_input, int):
                    data_equipment = (
                        CurriculaModel.objects.filter(
    ...
    CourseInformation_put_responses_success = Response(
        ...
        examples={"error_code": 0, "message": put_success},
    )
    CourseInformation_put_responses_fail = Response(
        description="添加课程信息失败的响应",
        schema=responses_fail,
        examples={"error_code": 1, "message": put_error},
    )
    ...
    def put(self, request, args, session):
        ...
        if isinstance(variable_name["id_location__name"], int) and isinstance(
            variable_name["id_speaker__name"], int
        ):
            variable_name["id_location"] = variable_name["id_location__name"]
            variable_name["id_speaker"] = variable_name["id_speaker__name"]
        else:
            return HttpResponse(
                dumps({"error_code": 1, "message": "请确保所填的id类数据是数字"}),
        ...
        try:
            ...
            speaker_object = UserModel.objects.get(id=variable_name.get("id_speaker"))
            idmanager_object = UserModel.objects.get(id=variable_name.get("idmanager"))
            ueses_tmp = CurriculaModel.objects.create(
            ...
            return HttpResponse(
                dumps({"error_code": 0, "message": put_success}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
    ...
    CourseInformation_patch_responses_success = Response(
        ...
        schema=responses_success,
        examples={"error_code": 0, "message": patch_success},
    )
    ...
    @swagger_auto_schema(
        ...
        security=None,
        responses={
            201: CourseInformation_patch_responses_success,
            400: CourseInformation_patch_responses_fail,
        },
        tags=None,
    )
    @get_request_args
    def patch(self, request, args, session):
        ...
        data_equipment_initial = list(
            CurriculaModel.objects.filter(id=id_equipment).values(
                "id",
                "name",
                "timebegin",
                "timeend",
        ...
        if data_equipment_initial == []:
            return HttpResponse(
                dumps({"error_code": 1, "message": id_error}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
        ...
        args["id_speaker"] = args.get("id_speaker__name", data_equipment["id_speaker"])
        args["idmanager"] = args.get("idmanager__name", data_equipment["idmanager"])
        if (
            isinstance(args["id_location__name"], int)
            and isinstance(args["id_speaker__name"], int)
            and isinstance(args["idmanager__name"], int)
        ):
        ...
        variable_name["idmanager"] = user_id
        try:
            CurriculaModel.objects.filter(id=id_equipment).update(
                id=variable_name.get("id"),
                name=variable_name.get("name"),
            ...
            return HttpResponse(
                dumps({"error_code": 0, "message": "修改课程表成功"}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
        except Exception as error:
    ...
    @swagger_auto_schema(
        ...
        operation_summary=None,
        security=None,
        responses={204: APIView_delete_request_body, 500: APIView_delete_request_body},
        tags=None,
    )
    @get_request_args
    def delete(self, request, args, session):
        ...
        try:
            for i in range(numbers_id):
                CurriculaModel.objects.filter(
                    id=variable_name.get("id_" + str(i), "id_1")
                ).delete()
                return HttpResponse(
                    dumps({"error_code": 0, "message": "数据删除成功"}),
                    content_type=content_type_tmp,
                    charset="utf-8",
                )
        except Exception as error:
