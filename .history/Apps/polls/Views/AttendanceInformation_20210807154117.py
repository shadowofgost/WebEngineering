...
class AttendanceInformation(APIView):
    ...
    def post(self, request, args, session):
        ...
        if input_string == None:
            ...
            if isinstance(test_input, int):
                data_equipment = list(models.RunningAccountModel.objects.filter(
                    Q(id=test_input) |
    ...
    def patch(self, request, args, session):
        ...
        data_equipment_initial = list(
            models.RunningAccountModel.objects.filter(id=id_equipment).values('id', 'id_user', 'param2', 'idmanager', 'timeupdate', 'param1', 'time', 'money', 'type_field'))
        ...
        variable_name['idmanager'] = user_id
        try:
            models.RunningAccountModel.objects.filter(id=id_equipment).update(
                id=variable_name.get('id'),
    ...
    def delete(self, request, args, session):
        ...
        try:
            for i in range(numbers_id):
                models.RunningAccountModel.objects.filter(
