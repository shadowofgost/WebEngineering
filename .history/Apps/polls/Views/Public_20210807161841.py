...
def data_attendance(course_plan_id, id_list, format_type, user_id):
    ...
    data_plan = [1, 2, 3, 4, 5]
    if format_type == 3:
        data_equipment =tuple(RunningAccountModel.objects.filter(id_user=user_id, param2__id_curricula=course_plan_id).values(
            'id', 'id_user__name', 'time', 'type_field', 'param2', 'timeupdate', 'idmanager__name').order_by('param2'))
        data_plan =tuple(CoursePlanModel.objects.filter(id_curricula=course_plan_id).values(
        ...
        data_equipment =tuple(RunningAccountModel.objects.filter(param2__id_curricula=course_plan_id).values(
