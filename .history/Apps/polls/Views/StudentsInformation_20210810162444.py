...
class StudentsInformation(APIView):
    ...
    data_schema = {
        ...
        "name": Schema(
            title="用户的姓名", description="用户的姓名", type=TYPE_STRING, format="string"
        ),
        "psw": Schema(
            title="用户的密码", description="用户的密码", type=TYPE_STRING, format="string"
        ),
        "deptid__name": Schema(
            title="部门", description="用户所属的部门名称", type=TYPE_STRING, format="string"
        ),
        "sex": Schema(
            title="性别",
        ...
        "localid": Schema(
            title="管理员地点", description=" 管理员所在的地点", type=TYPE_STRING, format="string"
        ),
        ...
        "userex_related_to_user_information__idmanager__name": Schema(
            title="管理员姓名", description="修改账户的管理员姓名", type=TYPE_STRING, format="string"
        ),
        "userex_related_to_user_information__rem": Schema(
            title="描述", description="照片的描述", type=TYPE_STRING, format="string"
        ),
    }
    ...
    StudentsInformation_get_responses_success = Response(
        description="学生个人信息查询成功的响应", schema=get_responses_success, examples=None
    )
    ...
    def get(self, request, args, session):
        ...
        data_user = (
            UserModel.objects.filter(attr=4)
            .values(
                "id",
                "nocard",
                "nouser",
    ...
    @swagger_auto_schema(
        ...
        security=None,
        responses={
            201: StudentsInformation_put_responses_success,
            400: StudentsInformation_put_responses_fail,
        },
        tags=None,
    )
    @get_request_args
    def put(self, request, args, session):
        ...
        try:
            deptid_object = DepartmentModel.objects.get(id=variable_name.get("deptid"))
            ...
            user_id_object = UserModel.objects.get(id=variable_name.get("id"))
            use_tmp_1 = UserExtensionModel.objects.create(
                id=user_id_object,
                rem=variable_name.get("userex_related_to_user_information__rem"),
                timeupdate=variable_name.get("timeupdate"),
    ...
    def patch(self, request, args, session):
        ...
        data_user_information_initial = list(
            UserModel.objects.filter(id=id_user_information).values(
                "id",
                "nocard",
                "nouser",
                "name",
                ...
                "localid",
                "userex_related_to_user_information__timeupdate",
                "userex_related_to_user_information__idmanager",
                "idmanager",
            )
        )
        ...
        variable_name = locals()
        args["deptid"] = args.get("deptid__name", data_user_information["deptid"])
        ...
        if isinstance(args["deptid__name"], int):
            ...
            args["userex_related_to_user_information__idmanager"] = args[
                "userex_related_to_user_information__idmanager__name"
            ]
        else:
            return HttpResponse(
        ...
        try:
            ...
            return HttpResponse(
                dumps({"error_code": 0, "message": "修改学生信息成功"}),
                content_type=content_type_tmp,
                charset="utf-8",
            )
        except Exception as error:
