...
class User(APIView):
    def get(self, request):
        data = models.UserModel.objects.all().values('name')
        data_result = []
...
class Academy(APIView):
    def get(self, request):
        user_id = '1910404051'
        data = models.UserModel.objects.get(id=user_id)
        data_department = models.DepartmentModel.objects.get(id=data.deptid)
        data_department = models.DepartmentModel.objects.get(id=data_department.parent)
        tmp = {
            "department_list": [
                {
...
def data_students_attendance(user_id, id_list):
    ...
    data_personal = list(models.RunningAccountModel.objects.filter(
    ...
    def post(self, request):
        ...
        if data == []:
            ...
            with connection.cursor() as cursor:
                ...
                data = cursor.fetchall()
            data_name = tuple(models.UserModel._meta.fields)
            data_result = []
            for i in data:
                tmp = dict(zip(data_name, i))
                data_result.append(tmp)
