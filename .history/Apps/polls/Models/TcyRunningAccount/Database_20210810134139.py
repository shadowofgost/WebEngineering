...
class RunningAccountModel(Model):
    ...
    """
    # 0表示缺席，1表示签到，2表示签退
    time = IntegerField(default=1, null=True, db_column="Time", blank=True)
    type_field = SmallIntegerField(default=1, null=True, db_column="Type", blank=True)
    ######type例如：交费、存款：0x101，赠费： 0x102，退费、取款：0x103，扣费、罚款：0x104，纠错，取消某次缴费、赠费等：0x106，上机费： 0x201,考勤： 0x1001######
    money = IntegerField(default=1, null=True, db_column="Money", blank=True)
    ######money发生的费用，单位为分#####
    param1 = IntegerField(default=1, null=True, db_column="Param1", blank=True)
