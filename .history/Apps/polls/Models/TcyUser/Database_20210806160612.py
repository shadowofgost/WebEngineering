from django.db.models import Model, CharField, ForeignKey, CASCADE,  IntegerField, ForeignKey, SmallIntegerField,OneToOneField,BinaryField,EmailField
from ..DepartmentModel.Database import DepartmentModel
class UserModel(Model):
    id = IntegerField(default=1, db_column='ID', primary_key=True)
    nocard = CharField(default='1', null=True,
                              db_column='Nocard', max_length=32)
    nouser = CharField(default='1', null=True,
                              db_column='NoUser', max_length=32)
    name = CharField(default='1', null=True,
                            db_column='Name', max_length=32)
    psw = CharField(default='1', null=True,
                           db_column='Psw', max_length=32)
    deptid =  ForeignKey(
        DepartmentModel, to_field="id", on_delete=CASCADE,  db_column='Deptid', related_name='related_to_department')
    ...
    ##用户余额2，单位为分；（扩展于特殊需求）##
    email = EmailField(default=None, null=True,
    ...
    idmanager = IntegerField(default=1, null=True,
...
class UserExtensionModel(Model):
    id = OneToOneField(
        UserModel, to_field="id", on_delete=CASCADE, primary_key=True, db_column='ID', related_name='userex_related_to_user_information')
    rem = CharField(default='1', null=True,
    ...
    idmanager = ForeignKey(
        UserModel, to_field="id", on_delete=CASCADE, db_column='IdManager', related_name='userex_related_to_user')
    imark = IntegerField(default=1, null=True, db_column='IMark')
    rem = CharField(default='1', null=True,
                           db_column='Rem', max_length=64)
    back_up1 = CharField(default='1', null=True,
