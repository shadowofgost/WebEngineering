from django.db.models import Model, CharField, ForeignKey, CASCADE,  IntegerField, ForeignKey, SmallIntegerField, OneToOneField, BigAutoField, Index
...
class LocationModel(Model):
    ...
    imark = SmallIntegerField(default=0, null=True,
                              db_column='IMark', blank=True)
    rem = CharField(default='0', null=True,
                    db_column='Rem', max_length=64, blank=True)
...
class LocationExtensionModel(Model):
    id_location = OneToOneField(
        LocationModel, to_field="id", on_delete=CASCADE, primary_key=True, db_column='ID_Location', related_name='locationex_related_to_location', db_constraint=False)
    attr = SmallIntegerField(
        default=1, null=True, db_column='Attr', blank=True)
    datebegin = IntegerField(
        default=1, null=True, db_column='DateBegin', blank=True)
