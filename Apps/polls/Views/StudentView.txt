...
def student_login(request):
    ...
    if request.method == "POST":
        ...
        if username and password:
            ...
            tmp = list(models.UserModel.objects.filter(nouser=username))
            try:
                user = models.UserModel.objects.get(nouser=username)
            except:
                return render(request, 'error/usernotexist.html')
            res_data = models.UserModel.objects.filter(
...
def student_changepsw(request):
    ...
    if is_login:
        ...
        result = {}
        dbData = models.UserModel.objects.filter(id=user_id)
        result['data'] = dbData
        if request.method == 'POST' and request.POST:
            username = username
            nameIndb = models.UserModel.objects.filter(id=user_id).first()
...
def get_course_information(request, location):
    ...
    if not(is_login):
        ...
        personname = request.session.get("personname", "")
        user_id = request.session.get("user_id")
        tmp = models.RunningAccountModel.objects.filter(id_user=user_id).values(
...
def student_lessoninfo(request, id):
    ...
    if is_login:
        ...
        personname = request.session.get("personname", "")
        user_id = request.session.get("user_id")
        tmp = models.CurriculaModel.objects.filter(
            id=id)
        lesson = tmp[0].name
        tmp_course_arrangement = models.CoursePlanModel.objects.filter(id_curricula=id)
        # for object in models.CoursePlanModel.objects.all():
        #    if username in object.rangeusers:
        #        if object.id_curricula == id:
        #            lesson = object
        #            lesson = models.CurriculaModel.objects.get#(id=lesson.id).name
        lessons = []
...
def student_attendtime(request, id):
    ...
    if is_login:
        ...
        #    object = models.RunningAccountModel.objects.get(id=object)
        #    object.time = time.strftime(
        #        "%Y-%m-%d %H:%M:%S", time.localtime(object.time))
        #    lessons.append(object)
        tmp = models.RunningAccountModel.objects.filter(
