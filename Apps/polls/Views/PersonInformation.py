    ...
    def patch(self, request, args, session):
        ...
        if data_patch_rem:
            data_user_extend = list(
                models.TCyuserex.objects.filter(id=user_id).values('id', 'rem', 'idmanager', 'timeupdate'))
            if data_user_extend == []:
                id_object = models.TCyuser.objects.get(id=user_id)
                models.TCyuserex.objects.create(
                    id=id_object,
                    rem='None',
